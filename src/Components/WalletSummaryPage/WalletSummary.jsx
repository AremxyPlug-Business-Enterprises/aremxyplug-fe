 ...
 const product = ["All Transactions", "Successful", "Failed","Pending", "Refunded", "Cancelled"]
  ...
  {/* Wallet summary */}
      <div className=" flex gap-[8px] md:gap-[10px] lg:mt-[25px] mt-[6px] md:mt-[10] flex-row">
          <div class="text-neutral-500 text-[11px] lg:text-[20px] md:text-[14px] font-semibold ">Wallet Summary</div>
  ...
  <img className="h-[16px] w-[14px] 
                  ...
                  <div
                    ...
                    >
                      ...
                      <p className="text-center text-[10px] leading-[13px] font-[500] 
                        lg:text-[18px] lg:leading-[24px]">
                        {selected === "NGN" ?  walletTransactionResponse?.data?.data?.data ?
                ...
                currency : "NGN"
                    ...
                    >
                      ...
                      <p className="text-center  text-[10px] leading-[13px] font-[500] 
                        ...
                        walletTransactionResponse?.data?.data?.data?.total_count
                         || walletTransactionResponse?.data?.status === 200  ?
                          walletTransactionResponse?.data?.data?.data?.total: "" : 0}  </p>
                    </div>
                    ...
                    >
                      ...
                      <p className="text-center  text-[10px] leading-[13px] font-[500] 
                        lg:text-[18px] lg:leading-[24px]">
                        {selected === "NGN"  ? walletTransactionResponse?.data?.data?.data ?
              walletTransactionResponse?.data?.data?.data?.total_outflow?.toLocaleString("en-NG", {
                style : "currency",
                currency : "NGN"
              }) :   "₦"  : `${symbolValue}0.00`}
                      </p>
   ...
   {/* product, quantity and total amount */}
      ...
      ${isDarkMode ? "border-white": "border-gray-500 "}`}
               ...
               filteredWalletTransactions && filteredWalletTransactions?.length > 1 ? (
          filteredWalletTransactions?.map((transaction, index) => (
                     <div className={`${index < filteredWalletTransactions?.length - 1 ? "border-b-[1px] border-gray-500 " : ""}`} 
                     key={index}>
                       <Link
                         to={`/${
            ...
            ) : transactionHistoryError === "unauthorised" && walletTransactionResponse?.data?.data?.data?.transactions === undefined ? (
                 <div className="h-[150px] flex items-center justify-center">
               <Loader/>
               </div>
