    ...
    const balanceStringToNum = Number(newBalance);
           ...
           const updateBalance = (newBalance === "" && passDataBalance?.data?.data?.data?.balance) ? Number(passDataBalance?.data?.data?.data?.balance) : balanceStringToNum;
             // const cleanUpBalanceToNumericOnly = Number(updateBalance?.replace(/\D/g, ""));
                       let showMaxAmountToNumber = Number(showMaxAmount);
             let CheckSufficiency =  showMaxAmountToNumber > updateBalance
            ...
            <div 
             className={` bvnQuery lg:rounded-[12px] rounded-[10px] 
              ...
              <div className="mx-auto">
                <p className="text-[12px] font-extrabold my-[5%] text-center
                 md:my-[3%] md:text-[15px] 
  ...
  <div className="flex flex-col gap-3 mt-5 md:mt-6 lg:mt-7">
             <div className="flex text-[10px] md:text-[14px] w-[90%] mx-auto 
                  ...
                  justify-between font-[500] lg:text-[16px]">
                    ...
                    <span className= {`${isDarkMode ? "text-white" : "text-black"}`}> {`${(showMaxAmount !== "" || showMaxAmount !== undefined) ? showMaxAmount?.toLocaleString("en-Ng",{
                      style : "currency",
                      currency : "NGN"
                    }): "₦"}`}</span>
                  </div>
