...
export default function EduSuccessReceipt() {
  const data = GetLocalStorage();
  const navigate = useNavigate();
  const {
    toggleSideBar, isDarkMode, orderIdResponse, setOrderIdResponse
  ...
  const quantity =
    receiptData?.quantity > 0 ? receiptData?.quantity : "";
  const amount =
    receiptData?.amount > 0 ? receiptData?.amount : "";
  const order_id =
  ...
  const description =
    receiptData?.transaction_description?.length > 0
      ...
      : "";
  const phone =
    receiptData?.phone_number?.length > 0
      ? receiptData?.phone_number
      : "";
  const email =
    receiptData?.email?.length > 0
      ? receiptData?.email
      : "";
  const pins_generated = receiptData?.pins_generated?.length > 0 ? receiptData?.pins_generated : "";
          ...
          <div ref={contentRef}>
            ...
            <p className="text-[10px] p-[5.729px] border-[0.573px] rounded-[6.302px] md:p-[5.868px] md:border-[0.578px] md:rounded-[6.455px]  lg:border lg:rounded-[11px] border-[#27AE60] leading-[15px] md:leading-[20px] text-[#27AE60] bg-[#D5F6E3] lg:p-2.5 text-center my-2 md:text-sm lg:text-base  lg:leading-6 font-medium md:mb-7">
              ...
              <span className=" font-extrabold text-[10.9px] md:text-[14.9px] lg:text-[16.9px]">
                <span className="uppercase">{exam_type}</span> (₦{String(amount).toLocaleString()}){" "}
              </span>
            ...
            >
              ...
              >
                ...
                <div className="flex text-[10px] md:text-sm w-[90%] mx-auto justify-between lg:text-base font-medium">
                  ...
                  >
                    <span className="uppercase">{exam_type}</span> PIN Generated
                  </p>
                  <span
                    className={` ${isDarkMode ? "text-white" : "text-black"}`}
                  >
