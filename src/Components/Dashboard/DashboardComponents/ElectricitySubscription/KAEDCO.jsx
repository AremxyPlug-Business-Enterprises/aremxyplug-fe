...
const KAEDCO = () => {
  ...
  const handleSuccess = async () => {
    async function buyKAEDCO(meter_type, meter_no, phone, email, amount) {
    ...
    // Usage
    const response = await buyKAEDCO(
              ...
              {showProductList && (
                <div className="border md:rounded-[10px] text-[10px] md:text-[12px] lg:text-[16px] mt-20 lg:mt-20 w-[90%] md:w-[44%]  rounded-[4px] absolute  bg-[#FFF] z-[10]">
                  {productList.map((item) => (
                    <div
                      key={item.name}
                      className={`pb-[14px] md:pb-0 pt-[14px] md:pt-0 font-weight-bold text-[14px] cursor-pointer border-b-[0.5px] text-[#7C7C7C] md:text-[12px] lg:text-[16px] w-[100%]  md:rounded-[0px] lg:mt-2 py-[4px] text-[12px] pl-[5px] ${selectedNetworkProduct === item.name ? "bg-white" : ""
                        }`}
        ...
        <Modal>
          ...
          >
            ...
            <div className="flex w-[70%] mx-auto items-center my-6  gap-[6%] md:gap-[20px] justify-center md:w-[20%] lg:my-[5%]">
              ...
              <Link to="/kaedco-receipt-failed" state={{
                selectedNetworkProduct: selectedNetworkProduct,
                meterNumber: meterNumber,
                phoneNumber: phoneNumber,
                ikedcEmail: ikedcEmail,
