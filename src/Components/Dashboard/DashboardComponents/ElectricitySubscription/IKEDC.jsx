...
const IKEDC = () => {
  ...
  const verifyMeterNumber = async (meterNumber) => {
      ...
      if (
        ...
        const SuccessHandler = () => {
          ...
          function handleReceivedMeterData() {
            if (ikedcFetchedResponse?.data?.name) {
              setIkedcCustomerName(ikedcFetchedResponse?.data?.name);
            } else {
              setIkedcCustomerName("");
            }
          }
    ...
    passedMeterName = ikedcFetchedResponse
      ? ikedcFetchedResponse?.data?.name
      : "";
  };

  const handleIkedcMeterNumber = async (e) => {
    const inputValue = e.target.value;
    setIkedcMeterNumber(inputValue);
  ...
  function handleReceivedData() {
    ...
    const receivedData = () => {
      ...
      setIkedcServiceID(ikedcFetchedResponse?.data?.RequestID);
      setIkedcShowDescription(
        ikedcFetchedResponse?.data?.transaction_description
      );
      setIkedcDiscoType(ikedcFetchedResponse?.data?.disco_type);
      setIkedcVerifiedName(ikedcFetchedResponse?.data?.verified_name);
      setIkedcFullName(ikedcFetchedResponse?.data?.full_name);
      setIkedcTransactionProduct(
        ikedcFetchedResponse?.data?.transaction_product
      );
    };
    receivedData();
    if (receivedData) {
    ...
    <DashBoardLayout>
      ...
      >
        <div>
          ...
          <div
            className={`flex lg:mt-[20px] sm:text-xs lg:text-base font-semibold pt-[30px] items-center ${
          ...
          <div className="lg:flex lg:items-start ">
            <div
              className={`mt-[10px] lg:mt-[15px] border font-bold text-[10px] lg:text-base lg:rounded-sm lg:py-2 text-center lg:px-3 py-1 
         ${
           isDarkMode
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 md:gap-6 lg:gap-6 items-center lg:mt-[20px] ">
            ...
            <div className="flex flex-col relative gap-2 lg:gap-2.5">
              ...
              {errors.ikedcPhoneNumber && (
                <div className="text-sm absolute left-0 -bottom-[1.5rem] leading-3 text-red-500 italic lg:text-sm">
                  {errors.ikedcPhoneNumber}
                </div>
              )}
            </div>
            <div className="flex flex-col gap-2 relative lg:gap-2.5">
              ...
              <div>
                <input
                  ...
                  onInput={() => {
                    setErrors((prev) => ({ ...prev, ikedcEmail: "" }));
                  }}
                  className={`w-full py-3 pl-[5.867px] lg:py-[14px] lg:pl-[10px] border md:py-3 md:pl-[8.67px] pr-1 md:pr-[5.867px] text-xs leading-[18px] border-[#9C9C9C] lg:text-base lg:leading-[20.8px] focus:outline-none placeholder:text-xs placeholder:leading-[10.4px] placeholder:lg:text-base placeholder:lg:leading-[20.8px] rounded-lg sm:rounded-[10px] h-full  ${
                    isDarkMode
                      ? "text-white bg-black border border-white"
            ...
            <div className="flex flex-col relative gap-2 lg:gap-2.5">
              ...
              {amountError && (
                <p className="text-sm text-red-500 italic lg:text-sm absolute left-0 -bottom-[1.3rem]">
                  {amountError}
                </p>
              )}
            </div>
        ...
        <footer className="flex justify-center items-center text-center gap-[18px] mt-[200px] pb-[10%] md:mt-[750px] lg:mt-[850px]">
          ...
          <Link to="/ContactUs">
            <div
              ...
              } text-[11px] px-2 py-1 text-white rounded-[8px] lg:text-base`}
            >
              Contact Us
            </div>
          </Link>
      ...
      {proceed && (
        <Modal>
          ...
          >
            ...
            <p
              className={`text-[10px] pt-[20px] md:pt-[10px] lg:pt-[20px] font-medium text-center mb-2 md:text-xs lg:text-sm ${
            ...
            <div className="flex flex-col gap-3 pt-[10px]">
              ...
              <div className="flex text-[10px] md:text-sm w-[90%] mx-auto justify-between  lg:text-base">
                ...
                <span>&#8358;{Number(ikedcAmount).toLocaleString()}</span>
              </div>
              <div className="flex text-[10px] md:text-sm w-[90%] mx-auto justify-between  lg:text-base">
                <p
                  className={`font-medium ${
